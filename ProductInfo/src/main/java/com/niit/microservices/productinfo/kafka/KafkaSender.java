package com.niit.microservices.productinfo.kafka;
...
public class KafkaSender {
	...
	@Autowired
	private KafkaTemplate<String, Item> jsonKafkaTemplate;
	...
	public void send(Item item, int partition) {
		...
		ListenableFuture<SendResult<String, Item>> future = null;
		if (partition != -1) {
			future = jsonKafkaTemplate.send(partitionTopic, partition, item);
		} else {
